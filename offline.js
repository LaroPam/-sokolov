(() => {
  // Assets (minimal procedural sprites)
  const COLOR_PALETTE = {
    playerIdle: '#d9ecde',
    playerWalk: '#c0e4cf',
    playerAttack: '#f5d36c',
  ...
  const SPRITE_SETS = {
    player: { idle: ['playerIdle'], walk: ['playerWalk1', 'playerWalk2'], attack: ['playerAttack'], hurt: ['playerHurt'] },
    enemies: {
    ...
    projectiles: { sword: 'projectileSword', knife: 'projectileKnife', bolt: 'projectileBolt', arrow: 'projectileArrow', orb: 'projectileOrb' },
  ...
  let game = null;
  let isBooting = false;
  // Let the player see and pick a weapon first; only build the cache once a
  // loadout is chosen.
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (startScreen.style.display !== 'none') {
        ...
        if (key >= '1' && key <= '4') {
          ...
          if (weapon) selectWeapon(weapon.id, weaponCardRefs[idx]?.card);
        }
      }
    });
  }
